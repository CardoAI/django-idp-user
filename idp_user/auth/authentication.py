...
from idp_user.utils.exceptions import AuthenticationError, MissingHeaders
...
# Allow header injection only if in Development Environment and the developers wants to inject to perform the injection.
INJECT_HEADERS = settings.IDP_USER_APP.get("INJECT_HEADERS", False) and settings.APP_ENV == 'development'
...
class AuthenticationBackend(authentication.TokenAuthentication):
    ...
    def __skip_auth_headers_and_opa(self, request: Request):
        try:
            ...
            return self._get_user(jwt_data['user_id'])
